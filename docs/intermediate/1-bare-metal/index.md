# Intermediate 1: Peripherals and Bare-Metal

## Learning Goals

- Program microcontrollers close to hardware registers.
- Configure and use common peripherals reliably.
- Handle interrupts and timing-sensitive behavior.

## Topics

- GPIO, timers, ADC, and interrupt handling
- Bare-metal firmware architecture
- STM32-focused practical implementation

## Recommended Videos

- [Bare Metal Embedded Programming: Theory and Practice Using STM32](https://www.youtube.com/playlist?list=PLP29wDx6HiE7S8p2W_vP_i8S-K1vW3c5O)
- [Fundamentals of Microcontrollers - Arduino bare-metal breakdown](https://www.youtube.com/playlist?list=PLNyfXcjhPpbVIsXWvE6p0wP65U39V3fU_)
